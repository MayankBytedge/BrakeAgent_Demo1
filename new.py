...
{{
    "rotor_diameter_mm": <value>,
    "pad_thickness_mm": <value>,
    "pad_type": "<organic/sintered/ceramic>",
    "optimization_explanation": "<brief explanation of changes made>"
}}
...
# Temperature simulation with enhanced physics
def simulate_temperature_and_wear(brake_params: Dict[str, Any], weather: str, 
    ...
    for step in range(steps + 1):
        ...
        if step > 0:
            for event in range(brake_events_per_step):
                ...
                # Convective cooling (from C++ code)
    ...
    # Calculate RUL with correction factor (hidden from UI)
    ...
    if total_wear_mm > 0:
        ...
        rul_km = max(8000, min(50000, int(remaining_life_km / BrakeConstants.RUL_CORRECTION_FACTOR)))
    else:
...
# Enhanced simulation for different speeds
...
def create_speed_based_temp_wear_chart(brake_params: Dict[str, Any]) -> go.Figure:
    """Create temperature vs wear chart for different speeds"""
...
def main():
    ...
    with st.sidebar:
        ...
        if GEMINI_API_KEY and st.session_state.ai_agent.model:
            ...
            st.warning("AI Agent: Limited (No API key)")
    ...
    with tabs[1]:
        ...
        if st.session_state.brake_params:
            ...
            # Seasonal tabs for weather analysis
